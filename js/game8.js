...
class FallingQuestion extends Drawable {
    constructor(canvasWidth, canvasHeight) {
        ...
        if(Math.random() > 0.5) {
            this.ans = "int";
            this.question = Math.floor(Math.random() * 100)-50;
        } else {
            this.ans = "float";
            this.question = Math.floor(Math.random() * 100)-50 + '.' + Math.floor(Math.random() * 10);
        }
    }
...
class Intro {
    constructor(container, callback) {
        this.container = container;
        this.container.innerHTML = "";
        this.container.className = "game8-intro-container";
        this.rule = document.createElement("img");
        this.rule.src = "img/game8-rule.jpg";
        this.rule.id = 'game8-rule';

        this.start = document.createElement("img");
        this.start.src = "img/game8-start.jpg";
        this.start.id = 'game8-start';
...
class Result {
    constructor(container, score, callback) {
        ...
        this.rule = document.createElement("img");
        if(score < 20) {
            this.rule.src = "img/game8-level1.jpg";
        } else if(score < 40) {
            this.rule.src = "img/game8-level2.jpg";
        } else {
            this.rule.src = "img/game8-level3.jpg";
        }
        this.rule.id = 'game8-rule';
        ...
        this.score.id = "game8-result-score";
        
        const congrats = new Audio('img/game8-congrats.m4a');
        congrats.play();
        this.start.onclick = () => {
            congrats.pause();
            callback();
...
class FallingQuestionGame extends AnimcationGame {
    constructor(container, data, callback) {
        ...
        this.container.className = "game8-container";
        this.callback = callback;

        this.intBtn = document.createElement("img");
        this.intBtn.src = "img/int-btn.jpg";
        this.intBtn.className = 'game8-btn';

        this.floatBtn = document.createElement("img");
        this.floatBtn.src = "img/float-btn.jpg";
        this.floatBtn.className = 'game8-btn';
        ...
        this.queue = [];
        this.audio = new Audio('img/game8-bg.m4a');
    ...
    start() {
        ...
        setInterval(() => {
            ...
            let fallingQuestion = new FallingQuestion(this.canvas.width, this.canvas.height);
